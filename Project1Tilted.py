...
def IMF(Mass):
	...
	C= Mass/((np.log(100)-np.log(0.1))*2970)
	N= lambda M1: C*np.log(M1)*np.diff(Mran)[1]
...
plt.yscale('log')
plt.savefig('InitalMassFunctionTilted.png')
plt.close()
...
T['O']=O
T['Fe']=Fe
T.write("OutputBigSB"+zf+".csv",overwrite=True)                               #Here
...
plt.savefig('ISMvzSB'+zf+'.png',dpi=300)                               #Here
...
plt.savefig('StarsvzSB'+zf+'.png',dpi=300)                               #Here
plt.close()
